...
def mouse_click(left=True, delay = 0.01):
    ...
    inp_down = INPUT(type=INPUT_MOUSE, union=_INPUTunion(
        mi=MOUSEINPUT(0, 0, 0, down_flag, 0, None)
    ))
    inp_up = INPUT(type=INPUT_MOUSE, union=_INPUTunion(
        mi=MOUSEINPUT(0, 0, 0, up_flag, 0, None)
    ))
